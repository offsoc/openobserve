...
async fn get_parquet_metadata(
    ...
    let start = file_size - parquet::file::FOOTER_SIZE as u64 - metadata_len as u64;
    let end = file_size - parquet::file::FOOTER_SIZE as u64;
    // read metadata
    let data = get_range(account, file, start..end).await?;
